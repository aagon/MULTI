...
Avec 256 sets, 16 mots par ligne, un degré d'associativité de 4, et un tampon d'écritures postées de 8 mots, le programme se termine à la fin du cycle 74502.

*** Question C2

Maintenant qu'on connaît le nombre de cycles nécessaires pour terminer le calcul, on peut demander 75000 cycles, et une période d'échantillonnage des statistiques de 1000.

Sur la totalité du calcul, on a un taux de miss sur le cache d'instructions de 0,06 %, et un taux de miss sur le cache de données de 0,1 %.

Si on regarde la totalité du calcul, les taux de miss sont décroissants [insérer graphique].

Si on regarde en particulier les 1000 premiers cycles, on a une décroissance, puis une stagnation, puis une décroissance à nouveau [insérer graphique].


Le CPI est aussi décroissant.


*** Question C3

Citer le code source, en donner la signification.
...
| Nombre de sets |  Durée | Taux de MISS | Coût de MISS | CPI |
|----------------+--------+--------------+--------------+-----|
|            256 |  85956 |              |              |     |
|             64 | 106270 |              |              |     |
|             16 |        |              |              |     |
...
Parce qu'on exécute pas le même programme, peut-être.

ce programme fait beaucoup plus d'entrées/sorties.


Le coût du miss qui est donné est une moyenne. Une moyenne de valeurs entières peut parfaitement être une valeur non entière.

Le remplissage du tampon d'écritures postées influera sur les coûts de tous les miss. Donc le coût du miss sera variable.
...
Si on augmente la taille de la ligne, on espère faire diminuer le taux de miss, mais on augmente le coût d'un miss.
Si on diminue la taille de la ligne, on espère faire diminuer le coût de miss, mais on augmente le taux d'un miss.

Le coût effectif du miss en nombre de cycles étant le produit du taux et du coût par miss, il est logique, par raison mathématique (on parle d'une fonction quadratique), que la configuration optimale sera sur les valeurs médianes.

[On va faire un joli graphique]

** Influence de la capacité du cache de données

| DSETS |  Durée | Taux miss D | Coût miss D |  CPI |
|-------+--------+-------------+-------------+------|
...
On peut faire un joli graphique.

** Influence de la profondeur du tampon d'écritures postées

*** Question G1

Le tampon d'écritures postées est un canal de communication FIFO à N cases.

Dans le tampon, on doit stocker :
- un bit de validité pour chaque case
- 30 bits d'adresse
...
Le coût des écritures, c'est le nombre de cycles de gel moyen occasionné par une écriture.

Ce coût est très faible, parce que pour occasionner un cycle de gel, il faut remplir le tampon. Puisque la vidange du tampon est prioritaire par rapport aux lectures, le tampon d'écritures postés n'occasionne de cycle de gel que si on a plus de n écritures consécutives avec n la taille du tampon. La probabilité d'avoir ces n écritures consécutives est en fait très faible (rappelons que en moyenne 10 % des instructions sont des instructions d'écriture), et diminue avec n augmentant.
